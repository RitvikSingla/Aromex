...
/**
 * Reads a company's accounts + current balances from Humble Ledger.
 *
 * HL's /api/v1/accounts returns the chart of accounts but **no balances** — the
 * balances live in the balance-sheet report. We use the date `2999-12-31` so the
 * report includes all posted entries and returns the true current balance,
 * sidestepping any timezone edge (per MOBILE_ADMIN_API.md §5).
 *
 * On a 401 from HL we invalidate the cached HL token once, ask the provider for
 * a fresh one, and retry the call exactly once before surfacing the failure.
 */
class HlLedgerRepository(
    ...
    private fun parse(resp: Response, attempt: Int): List<LedgerAccount> {
        if (!resp.isSuccessful) {
            ...
            NetLog.w(TAG, "attempt $attempt → HTTP ${resp.code} (not 2xx); body=$errBody")
            throw HlException(HlError.HlUnreachable)
        }
        val text = resp.body?.string().orEmpty()
        NetLog.logLong(TAG, "attempt $attempt → HTTP ${resp.code}; raw body:", text)
        val envelope = try {
        ...
        out.forEach { acc ->
            NetLog.d(TAG, "  • ${acc.category}/${acc.type}  ${acc.name} = ${acc.balance}  [id=${acc.id}]")
        }
        return out
    }

    @Serializable
    private data class BalanceSheetEnvelope(
        val success: Boolean = false,
        val data: BalanceSheetData? = null,
    )
    ...
    @Serializable
    private data class BalanceSheetRow(
